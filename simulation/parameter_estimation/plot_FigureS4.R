...
plot2 <- g + geom_col(aes(fill = Method,y= RMSE_A), position = position_dodge())+
  scale_fill_manual(values = c("#df7676","#79add2", "#13679e", "#3c5587","#f09b81"))+
  ylab("Relative RMSE")+
  theme_bw()+ 
  theme(legend.position = "none")+
...
g <- ggplot(plot_s15, aes(Method))
plot3 <- g + geom_col(aes(fill = Method,y= RMSE_A), position = position_dodge())+
  scale_fill_manual(values = c("#df7676","#79add2", "#13679e", "#3c5587","#f09b81"))+
  ylab("Relative RMSE")+
  theme_bw()+ 
  theme(legend.position = "none")+
...
plot_FigureS4 <- plot_grid(plot1,plot2,plot3, ncol=3, nrow=1,labels = c("A","B", "C"))
ggsave(filename="plot_FigureS4.png",plot=plot_FigureS4,device="png",dpi=600,units="in",width=18,height=6)
