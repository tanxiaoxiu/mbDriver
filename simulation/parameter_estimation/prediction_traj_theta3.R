...
path <- sprintf("~/mbDriver/simulation/parameter_estimation/trajectory/theta3/%s", setting)
setwd(path)
theta0 = 3
if(setting == "p10") {
  p <- 10
...
base_path1 <- "~/mbDriver/simulation/parameter_estimation/Supplementary"
base_path2 <- "~/mbDriver/simulation/parameter_estimation/Supplementary/other_methods"

load(paste0(base_path1, "/", setting, "/t13/theta3/A0_tables.Rdata"))
load(paste0(base_path1, "/", setting, "/t13/theta3/r0_tables.Rdata"))

load(paste0(base_path1, "/", setting, "/t13/theta3/sp_A_ridge_tables.Rdata"))
load(paste0(base_path1, "/", setting, "/t13/theta3/sp_r_ridge_tables.Rdata"))

load(paste0(base_path2, "/", setting, "/t13/theta3/BAL_list.Rdata"))
load(paste0(base_path2, "/", setting, "/t13/theta3/BVS_list.Rdata"))
load(paste0(base_path2, "/", setting, "/t13/theta3/MLRR_list.Rdata"))
load(paste0(base_path2, "/", setting, "/t13/theta3/MLCRR_list.Rdata"))
...
#r_rmse/corr
mean_r_rmse_all_k <- lapply(r_rmse_list, function(x) colMeans(x, na.rm = TRUE))
mean_corr_all_k <- lapply(corr_list, function(x) colMeans(x, na.rm = TRUE))

mean_r_rmse <- Reduce(`+`, mean_r_rmse_all_k) / length(mean_r_rmse_all_k)
mean_corr <- Reduce(`+`, mean_corr_all_k) / length(mean_corr_all_k)

r_rmse_corr <- cbind(mean_r_rmse,mean_corr)
r_rmse_corr <- adjustdata(r_rmse_corr)
colnames(r_rmse_corr) <- c("Method","r_rmse","corr")
...
write.table(r_rmse_corr, file = rmse_corr_txt_file_name, row.names = FALSE, col.names = TRUE, sep = "\t", quote = FALSE)