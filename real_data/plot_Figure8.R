...
Driver_UC1 <- pathway_species_UC1[,c("time","Lipid metabolism","Amino acid metabolism",
                                     "Desulfovibrionaceae","Ruminococcaceae","Porphyromonadaceae",
...
Driver_UC1_scale[,2:8] <- log(Driver_UC1_scale[,2:8]+1)
S <- Driver_UC1_scale$subject

###Lipid_metabolism
model.f1 <- lmer(Lipid_metabolism ~ Desulfovibrionaceae + Ruminococcaceae + Porphyromonadaceae +
                   Lachnospiraceae + Bacteroidaceae  + (1|S), data = Driver_UC1_scale, REML=FALSE)
...
model.f3 <- lmer(Amino_acid_metabolism ~ Desulfovibrionaceae + Ruminococcaceae + Porphyromonadaceae +
                   Lachnospiraceae + Bacteroidaceae + (1|S), data = Driver_UC1_scale, REML=FALSE)
...
dt4plot$class <- gsub("UC1:", "", dt4plot$class)
...
ggsave(filename = "Figure8C.png", plot=p2, width=4, height = 6, dpi=600)
