...
#write.table(OTU,file ="OTU.txt",row.names = T,col.names = T, sep = "\t",quote = F)
...
p1
ggsave(filename="FigureS10A.png",plot=p1,device="png",dpi=600,units="in",width=6,height=5)
...
p2 <- ggplot(df2,aes(x=Group,y=Simpson))+
  ...
  theme(legend.position = 'none')
p2
ggsave(filename="FigureS10B.png",plot=p2,device="png",dpi=600,units="in",width=6,height=5)
...
p3
ggsave(filename="FigureS10C.png",plot=p3,device="png",dpi=600,units="in",width=6,height=5)
