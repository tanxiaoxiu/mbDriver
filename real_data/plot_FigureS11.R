...
setwd("~/mbDriver/real_data/other/UC")
...
Driver_UC1 <- pathway_species_UC1[,c("time","Lipid metabolism","Amino acid metabolism",
                                     "Porphyromonadaceae","Prevotellaceae","Desulfovibrionaceae",
                                     "Sutterellaceae",
                                     "group","subject")]
...
Driver_UC1_scale[,2:7] <- log(Driver_UC1_scale[,2:7]+1)
S <- Driver_UC1_scale$subject

###Lipid_metabolism
model.f1 <- lmer(Lipid_metabolism ~ Porphyromonadaceae + Prevotellaceae + Desulfovibrionaceae + Sutterellaceae + (1|S), data = Driver_UC1_scale, REML=FALSE)
...
model.f3 <- lmer(Amino_acid_metabolism ~ Porphyromonadaceae + Prevotellaceae + Desulfovibrionaceae + Sutterellaceae + (1|S), data = Driver_UC1_scale, REML=FALSE)
...
write.table(UC_scatter_data,file ="UC_scatter_data_mdsine_UC1.txt",row.names = F,col.names = T, sep = "\t",quote = F)

##plot
##Figure S11
infile <- "UC_scatter_data_mdsine_UC1.txt"
...
ggsave(filename = "FigureS11.png", plot=p2, width=8, height = 2, dpi=600)
